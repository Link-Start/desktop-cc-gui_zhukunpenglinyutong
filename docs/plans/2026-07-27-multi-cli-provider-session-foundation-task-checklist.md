...
| R.6 | Continuation 可读投影校准 | 不再把校验 hash、协议 marker 和“default”直接扔给用户看。 | 已知 `MOSSX_CONTEXT_*` control message 从幕布隐藏；协议标题投影为“继续：来源会话”；续接会话顶部显示来源/目标卡片，并可打开来源；本地 Provider 显示“本地配置”。 | **有**：Sidebar 标题可读、续接标签有解释、幕布顶部可回来源；普通包含 MOSSX 字样的用户消息不会被误删。 | → | D.3、D.4 | exact marker classifier、title projection、source navigation、missing-source fallback tests | M | ✅ 已完成 |
