...
> **内容类型**：How-to / Checklist
> **生命周期**：active
> **最后校准**：2026-08-03 · mossx `0.7.16`
> **事实源**：`src-tauri/build.rs`、`src-tauri/tauri.conf.json`、`skills-lock.json`、`src-tauri/src/curated_skills.rs`
> **更新触发器**：Metadata schema、build whitelist、resource mapping、runtime kill switch 或 lock format 变化
...
`Sparkles` (PascalCase) / emoji / 任何非 ASCII 一律 reject。当前 build gate 不校验 lucide icon 名是否真实存在，PR review 仍需人工确认。
...
扩充 category 前必须先修改 `build.rs` 的 canonical enum，并通过独立 OpenSpec change 评审。
...
1. `src-tauri/tauri.conf.json` 的 `bundle.resources` 移除两条:
   ...
   - `"../skills-lock.json": "."`
2. `skills-lock.json` 删除所有 `kind: "curated"` entry
...
本文不维护 product backlog。Icon whitelist、license、category、token estimation 或 picker behavior 的演进必须进入 active OpenSpec change；没有 active change 时不得把历史 proposal item 当作待办。
