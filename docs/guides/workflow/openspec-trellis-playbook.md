...
> **事实源**：`AGENTS.md`、`.trellis/workflow.md`、`openspec/README.md`
> **更新触发器**：Trellis task/session lifecycle、OpenSpec CLI 或 commit gate 变化
...
3. 开发与自测。
4. 提交代码；成功 commit 后立即执行 Trellis session record。
5. 运行 OpenSpec 校验并完成 sync/archive（按发布策略）。
6. PR 中写清映射关系、code commit、record commit 与验证证据。
...
# 成功 commit 后记录 session；先读取命令生成的 developer context
python3 ./.trellis/scripts/get_context.py --mode record
printf '%s\n' '<session-summary>' | python3 ./.trellis/scripts/add_session.py \
  --stdin --title "<title>" --commit "<code-commit>"
