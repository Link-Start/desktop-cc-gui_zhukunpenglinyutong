---
type: evidence
status: historical
---

# v0.5.11 Performance Baseline

> **Lifecycle**：Immutable Historical Generated Snapshot。仅用于对应 version/commit 的回归比较，不代表当前 HEAD。

Generated at: 2026-06-17T16:22:56.825Z
Schema version: 1.0
Branch: feature/v0.5.11
Commit: 7706f6270784efed25e8aa28e992b5d8b6d88d3f

## Section A — Fixture-Replay Baseline

| Scenario | Metric | Value | Unit | Evidence | Target | Hard Fail | Notes |
|---|---:|---:|---|---|---:|---:|---|
| S-LL-200 | commitDurationP50 | 10.06 | ms | proxy |  |  |  |
| S-LL-200 | commitDurationP95 | 10.06 | ms | proxy |  |  |  |
| S-LL-200 | firstPaintAfterMount | 36.9 | ms | proxy |  |  |  |
| S-LL-500 | commitDurationP50 | 12.96 | ms | proxy |  |  |  |
| S-LL-500 | commitDurationP95 | 12.96 | ms | proxy |  |  |  |
| S-LL-500 | firstPaintAfterMount | 32.03 | ms | proxy |  |  |  |
| S-LL-1000 | commitDurationP50 | 21.28 | ms | proxy |  |  |  |
| S-LL-1000 | commitDurationP95 | 21.28 | ms | proxy |  |  |  |
| S-LL-1000 | firstPaintAfterMount | 41.55 | ms | proxy |  |  |  |
| S-LL-1000 | scrollFrameDropPct | 0 | % | proxy | 1 | 5 | jsdom proxy; browser scroll gate is follow-up |
| S-CI-50 | keystrokeToCommitP95 | 0.08 | ms | proxy | 16 | 32 |  |
| S-CI-50 | inputEventLossCount | 0 | count | proxy | 0 | 0 |  |
| S-CI-50 | compositionToCommit | 0 | ms | proxy |  |  |  |
| S-CI-100-IME | keystrokeToCommitP95 | 0.03 | ms | proxy | 16 | 32 |  |
| S-CI-100-IME | inputEventLossCount | 0 | count | proxy | 0 | 0 |  |
| S-CI-100-IME | compositionToCommit | 0.1 | ms | proxy |  |  |  |
| S-RS-FT | firstTokenLatency | 5000 | ms | proxy | 2000 | 5000 | turn start to first assistant delta |
| S-RS-FT | interTokenJitterP95 | 920 | ms | proxy | 500 | 920 |  |
| S-RS-PE | dedupHitRatio | 0.25 | ratio | proxy |  |  |  |
| S-RS-PE | assemblerLatency | 4.21 | ms | proxy |  |  | replay reducer-path proxy latency |
| S-RS-VL | visibleTextLagP95 | 24 | ms | proxy | 2000 | 5000 | first engine delta ingress -> first visible text growth (per-turn P95, turn-trace correlation gate) |
| S-RS-RA | reducerAmplificationMedian | 1.33 | ratio | proxy | 2 | 4 | reducer commit count / delta count (median across completed turns) |
| S-RS-FD | batchFlushDurationP95 | 13.33 | ms | proxy | 8 | 16 | batch flush duration P95 across completed turns |
| S-RS-TS | terminalSettlementP95 | 60 | ms | proxy | 100 | 250 | last reducer commit -> terminal settlement P95 across completed turns |
| S-CS-COLD | bundleSizeMain | 1068094 | bytes-gzip | measured | 950000 | 1100000 | App-pCMuvY3q.js |
| S-CS-COLD | bundleSizeVendor | 741553 | bytes-gzip | measured | 680000 | 760000 | subset-shared.chunk-BcpPBYlL.js |
| S-CS-COLD | firstPaintMs | unsupported | ms | unsupported |  |  | Tauri/webview startup marker snapshot was not provided; bundle baseline is recorded. |
| S-CS-COLD | firstInteractiveMs | unsupported | ms | unsupported |  |  | Tauri/webview startup marker snapshot was not provided; bundle baseline is recorded. |

## Section B — Cross-Platform Notes

- darwin: S-CS-COLD/firstPaintMs unsupported - Tauri/webview startup marker snapshot was not provided; bundle baseline is recorded.
- darwin: S-CS-COLD/firstInteractiveMs unsupported - Tauri/webview startup marker snapshot was not provided; bundle baseline is recorded.

## Section C — Previous Baseline Comparison

Previous baseline: v0.5.6 (docs/perf/history/v0.5.6-baseline.json)

| Scenario | Metric | Previous | Current | Delta | Unit | Evidence | Status |
|---|---|---:|---:|---:|---|---|---|
| S-LL-200 | commitDurationP50 | 17.6 | 10.06 | -7.54 | ms | proxy | comparable |
| S-LL-200 | commitDurationP95 | 17.6 | 10.06 | -7.54 | ms | proxy | comparable |
| S-LL-200 | firstPaintAfterMount | 54.54 | 36.9 | -17.64 | ms | proxy | comparable |
| S-LL-500 | commitDurationP50 | 170.66 | 12.96 | -157.7 | ms | proxy | comparable |
| S-LL-500 | commitDurationP95 | 170.66 | 12.96 | -157.7 | ms | proxy | comparable |
| S-LL-500 | firstPaintAfterMount | 195.04 | 32.03 | -163.01 | ms | proxy | comparable |
| S-LL-1000 | commitDurationP50 | 34.47 | 21.28 | -13.19 | ms | proxy | comparable |
| S-LL-1000 | commitDurationP95 | 34.47 | 21.28 | -13.19 | ms | proxy | comparable |
| S-LL-1000 | firstPaintAfterMount | 58.43 | 41.55 | -16.88 | ms | proxy | comparable |
| S-LL-1000 | scrollFrameDropPct | 0 | 0 | 0 | % | proxy | comparable |
| S-CI-50 | keystrokeToCommitP95 | 0.08 | 0.08 | 0 | ms | proxy | comparable |
| S-CI-50 | inputEventLossCount | 0 | 0 | 0 | count | proxy | comparable |
| S-CI-50 | compositionToCommit | 0 | 0 | 0 | ms | proxy | comparable |
| S-CI-100-IME | keystrokeToCommitP95 | 0.03 | 0.03 | 0 | ms | proxy | comparable |
| S-CI-100-IME | inputEventLossCount | 0 | 0 | 0 | count | proxy | comparable |
| S-CI-100-IME | compositionToCommit | 0.11 | 0.1 | -0.01 | ms | proxy | comparable |
| S-RS-FT | firstTokenLatency | 5000 | 5000 | 0 | ms | proxy | comparable |
| S-RS-FT | interTokenJitterP95 | 920 | 920 | 0 | ms | proxy | comparable |
| S-RS-PE | dedupHitRatio | 0.25 | 0.25 | 0 | ratio | proxy | comparable |
| S-RS-PE | assemblerLatency | 5.73 | 4.21 | -1.52 | ms | proxy | comparable |
| S-RS-VL | visibleTextLagP95 | unsupported | 24 |  | ms | proxy | missing |
| S-RS-RA | reducerAmplificationMedian | unsupported | 1.33 |  | ratio | proxy | missing |
| S-RS-FD | batchFlushDurationP95 | unsupported | 13.33 |  | ms | proxy | missing |
| S-RS-TS | terminalSettlementP95 | unsupported | 60 |  | ms | proxy | missing |
| S-CS-COLD | bundleSizeMain | unsupported | 1068094 |  | bytes-gzip | measured | missing |
| S-CS-COLD | bundleSizeVendor | unsupported | 741553 |  | bytes-gzip | measured | missing |
| S-CS-COLD | firstPaintMs | unsupported | unsupported |  | ms | unsupported | not comparable |
| S-CS-COLD | firstInteractiveMs | unsupported | unsupported |  | ms | unsupported | not comparable |

> Comparison status: 20/28 metrics comparable; 6 missing, 2 not comparable.

## Section D — Residual Risks

- Baseline values are fixture-based and should be used for relative comparison, not absolute UX claims.
