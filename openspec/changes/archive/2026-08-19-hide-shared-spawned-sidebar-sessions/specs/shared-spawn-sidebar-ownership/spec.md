...
Claude Session Index writer MUST 在入库前检查 jsonl 首条真实 user 正文。若正文以 `MOSSX_CONTEXT_PACKAGE` / `MOSSX_SHARED_CONTEXT_V1` / `MOSSX_NATIVE_CONTEXT_V1` 开头，该 session MUST NOT 写入 Index。`history.jsonl` 的友好 display 标题（如「继续」）MUST NOT 覆盖此 omit。

protocol hide MUST 收录这些被 omit / 协议拥有的 **文件 sessionId**（以及 `claude:{fileUuid}`），使子代理 parent 能命中 hide set。
...
- **THEN** Session Index MUST NOT 为该文件 sessionId 写入侧栏行
