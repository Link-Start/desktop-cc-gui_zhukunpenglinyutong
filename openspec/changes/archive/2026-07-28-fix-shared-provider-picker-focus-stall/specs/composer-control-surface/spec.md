...
Provider Profile accordion 时，root selector 与当前 CLI submenu MUST 保持打开，且
repeated interaction MUST NOT 因 open/focus state 竞争而 stall。
...
#### Scenario: repeated shared provider interaction remains responsive

- **WHEN** 用户连续展开、折叠或在多个 Shared Provider Profiles 之间快速切换
- **THEN** 每次 interaction MUST 收敛到最后一次请求的 expanded state
- **AND** selector MUST NOT freeze、重复 toggle 或进入 open/close update loop
