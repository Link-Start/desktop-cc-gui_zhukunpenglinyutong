...
While the latest DSH `goal/change` snapshot for a bound session has
`phase === "active"`, mossx SHALL NOT emit `TurnCompleted` for a completed
hop `turn/end`. The composer MUST remain in a running / processing state so
the host-owned Goal loop can start the next hop.
