...
When a DSH turn includes image parts and the selected model is served by a
writable `llm-pi-ai` provider route that has not declared image input, mossx
MUST declare `[text, image]` through Host RPC `settings.mutate` before
`session.prompt`. mossx MUST NOT require the user to edit
`$DSH_HOME/settings.yaml` or open the DSH Web UI for this declaration.
