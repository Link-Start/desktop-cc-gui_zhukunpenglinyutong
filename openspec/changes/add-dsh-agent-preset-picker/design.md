...
锁定口径：`selectedEngine === "dsh"` 且当前 DSH 会话已有 **user** message。首页 create-session 与 `dsh-pending-*`（无用户消息）仍算空白。tool / stats / explore 不得把空白会话锁死。

Resume 展示：`session.list.agentPreset` 写入 `ThreadSummary.dshAgentPreset` 后，index / live merge 必须保留该字段。index 行没有 preset 时不得覆盖 live header。
