import type {
  DarkThemePresetId,
  LightThemePresetId,
  ThemeAppearance,
  ThemePresetId,
} from "../../../types";

export type VsCodeThemeColors = Record<string, string>;

export type SyntaxTokens = {
  keyword: string;
  string: string;
  comment: string;
  number: string;
  function: string;
  operator: string;
  type: string;
  tag: string;
};

export type DiffTokens = {
  inserted: string;
  removed: string;
};

export type VsCodeThemePresetDefinition = {
  id: ThemePresetId;
  appearance: ThemeAppearance;
  labelKey: string;
  colors: VsCodeThemeColors;
  syntax?: SyntaxTokens;
  diff?: DiffTokens;
};

export const DEFAULT_LIGHT_THEME_PRESET_ID: LightThemePresetId =
  "vscode-light-modern";
export const DEFAULT_DARK_THEME_PRESET_ID: DarkThemePresetId =
  "vscode-dark-modern";

export const LIGHT_THEME_PRESET_IDS = [
  "vscode-light-modern",
  "vscode-light-plus",
  "vscode-github-light",
  "vscode-solarized-light",
  "vscode-catppuccin-latte",
  "vscode-tokyo-day",
  "vscode-rose-pine-dawn",
  "vscode-everforest-light",
  "vscode-ayu-light",
] as const satisfies readonly LightThemePresetId[];

export const DARK_THEME_PRESET_IDS = [
  "vscode-dark-modern",
  "vscode-dark-plus",
  "vscode-github-dark",
  "vscode-github-dark-dimmed",
  "vscode-one-dark-pro",
  "vscode-monokai",
  "vscode-solarized-dark",
  "vscode-dracula",
  "vscode-nord",
  "vscode-catppuccin-mocha",
  "vscode-tokyo-night",
  "vscode-rose-pine",
] as const satisfies readonly DarkThemePresetId[];

export const ALL_THEME_PRESET_IDS = [
  ...LIGHT_THEME_PRESET_IDS,
  ...DARK_THEME_PRESET_IDS,
] as const satisfies readonly ThemePresetId[];

const DARK_MODERN_COLORS: VsCodeThemeColors = {
  foreground: "#cccccc",
  descriptionForeground: "#9d9d9d",
  "editor.background": "#1f1f1f",
  "editor.foreground": "#cccccc",
  "sideBar.background": "#181818",
  "sideBar.foreground": "#cccccc",
  "panel.background": "#181818",
  "panel.border": "#2b2b2b",
  "input.background": "#313131",
  "input.foreground": "#cccccc",
  "input.border": "#3c3c3c",
  "button.background": "#0078d4",
  "button.foreground": "#ffffff",
  "button.secondaryBackground": "#313131",
  "button.secondaryForeground": "#cccccc",
  "dropdown.background": "#313131",
  "dropdown.border": "#3c3c3c",
  "editorWidget.background": "#202020",
  "list.hoverBackground": "#242424",
  "list.activeSelectionBackground": "#04395e",
  "list.activeSelectionForeground": "#ffffff",
  "activityBar.background": "#181818",
  "activityBar.foreground": "#d7d7d7",
  "statusBar.background": "#181818",
  "statusBar.foreground": "#cccccc",
  "titleBar.activeBackground": "#181818",
  "titleBar.activeForeground": "#cccccc",
  "terminal.background": "#1f1f1f",
  "terminal.foreground": "#cccccc",
  "terminal.selectionBackground": "#264f78",
  "terminalCursor.foreground": "#cccccc",
  "editorLineNumber.foreground": "#6e7681",
  "editorGutter.addedBackground": "#2ea043",
  "editorGutter.modifiedBackground": "#0078d4",
  "editorGutter.deletedBackground": "#f85149",
  "textLink.foreground": "#4daafc",
  "badge.background": "#616161",
  "badge.foreground": "#f8f8f8",
};

const DARK_PLUS_COLORS: VsCodeThemeColors = {
  foreground: "#cccccc",
  descriptionForeground: "#969696",
  "editor.background": "#1e1e1e",
  "editor.foreground": "#d4d4d4",
  "sideBar.background": "#252526",
  "sideBar.foreground": "#cccccc",
  "panel.background": "#1e1e1e",
  "panel.border": "#2b2b2b",
  "input.background": "#3c3c3c",
  "input.foreground": "#cccccc",
  "input.border": "#3c3c3c",
  "button.background": "#0e639c",
  "button.foreground": "#ffffff",
  "button.secondaryBackground": "#3a3d41",
  "button.secondaryForeground": "#cccccc",
  "dropdown.background": "#3c3c3c",
  "dropdown.border": "#3c3c3c",
  "editorWidget.background": "#252526",
  "list.hoverBackground": "#2a2d2e",
  "list.activeSelectionBackground": "#094771",
  "list.activeSelectionForeground": "#ffffff",
  "activityBar.background": "#333333",
  "activityBar.foreground": "#ffffff",
  "statusBar.background": "#007acc",
  "statusBar.foreground": "#ffffff",
  "titleBar.activeBackground": "#3c3c3c",
  "titleBar.activeForeground": "#cccccc",
  "terminal.background": "#1e1e1e",
  "terminal.foreground": "#cccccc",
  "terminal.selectionBackground": "#264f78",
  "terminalCursor.foreground": "#aeafad",
  "editorLineNumber.foreground": "#858585",
  "editorGutter.addedBackground": "#2ea043",
  "editorGutter.modifiedBackground": "#007acc",
  "editorGutter.deletedBackground": "#f14c4c",
  "textLink.foreground": "#3794ff",
  "badge.background": "#4d4d4d",
  "badge.foreground": "#ffffff",
};

const LIGHT_MODERN_COLORS: VsCodeThemeColors = {
  foreground: "#3b3b3b",
  descriptionForeground: "#3b3b3b",
  "editor.background": "#ffffff",
  "editor.foreground": "#3b3b3b",
  "sideBar.background": "#f8f8f8",
  "sideBar.foreground": "#3b3b3b",
  "panel.background": "#f8f8f8",
  "panel.border": "#e5e5e5",
  "input.background": "#ffffff",
  "input.foreground": "#3b3b3b",
  "input.border": "#cecece",
  "button.background": "#005fb8",
  "button.foreground": "#ffffff",
  "button.secondaryBackground": "#e5e5e5",
  "button.secondaryForeground": "#3b3b3b",
  "dropdown.background": "#ffffff",
  "dropdown.border": "#cecece",
  "editorWidget.background": "#f8f8f8",
  "list.hoverBackground": "#f2f2f2",
  "list.activeSelectionBackground": "#e8e8e8",
  "list.activeSelectionForeground": "#000000",
  "activityBar.background": "#f8f8f8",
  "activityBar.foreground": "#1f1f1f",
  "statusBar.background": "#f8f8f8",
  "statusBar.foreground": "#3b3b3b",
  "titleBar.activeBackground": "#f8f8f8",
  "titleBar.activeForeground": "#1e1e1e",
  "terminal.background": "#ffffff",
  "terminal.foreground": "#3b3b3b",
  "terminal.selectionBackground": "#add6ff",
  "terminalCursor.foreground": "#005fb8",
  "editorLineNumber.foreground": "#6e7681",
  "editorGutter.addedBackground": "#2ea043",
  "editorGutter.modifiedBackground": "#005fb8",
  "editorGutter.deletedBackground": "#f85149",
  "textLink.foreground": "#005fb8",
  "badge.background": "#cccccc",
  "badge.foreground": "#3b3b3b",
};

const LIGHT_PLUS_COLORS: VsCodeThemeColors = {
  foreground: "#3b3b3b",
  descriptionForeground: "#616161",
  "editor.background": "#ffffff",
  "editor.foreground": "#000000",
  "sideBar.background": "#f3f3f3",
  "sideBar.foreground": "#3b3b3b",
  "panel.background": "#ffffff",
  "panel.border": "#d4d4d4",
  "input.background": "#ffffff",
  "input.foreground": "#3b3b3b",
  "input.border": "#cecece",
  "button.background": "#007acc",
  "button.foreground": "#ffffff",
  "button.secondaryBackground": "#ececec",
  "button.secondaryForeground": "#000000",
  "dropdown.background": "#ffffff",
  "dropdown.border": "#cecece",
  "editorWidget.background": "#f3f3f3",
  "list.hoverBackground": "#e8e8e8",
  "list.activeSelectionBackground": "#cce6ff",
  "list.activeSelectionForeground": "#000000",
  "activityBar.background": "#f3f3f3",
  "activityBar.foreground": "#424242",
  "statusBar.background": "#007acc",
  "statusBar.foreground": "#ffffff",
  "titleBar.activeBackground": "#dddddd",
  "titleBar.activeForeground": "#333333",
  "terminal.background": "#ffffff",
  "terminal.foreground": "#3b3b3b",
  "terminal.selectionBackground": "#add6ff",
  "terminalCursor.foreground": "#000000",
  "editorLineNumber.foreground": "#237893",
  "editorGutter.addedBackground": "#2ea043",
  "editorGutter.modifiedBackground": "#007acc",
  "editorGutter.deletedBackground": "#cd3131",
  "textLink.foreground": "#006ab1",
  "badge.background": "#c4c4c4",
  "badge.foreground": "#333333",
};

const GITHUB_LIGHT_COLORS: VsCodeThemeColors = {
  foreground: "#1f2328",
  descriptionForeground: "#57606a",
  "editor.background": "#ffffff",
  "editor.foreground": "#1f2328",
  "sideBar.background": "#f6f8fa",
  "sideBar.foreground": "#1f2328",
  "panel.background": "#ffffff",
  "panel.border": "#d0d7de",
  "input.background": "#ffffff",
  "input.foreground": "#1f2328",
  "input.border": "#d0d7de",
  "button.background": "#0969da",
  "button.foreground": "#ffffff",
  "button.secondaryBackground": "#f6f8fa",
  "button.secondaryForeground": "#1f2328",
  "dropdown.background": "#ffffff",
  "dropdown.border": "#d0d7de",
  "editorWidget.background": "#ffffff",
  "list.hoverBackground": "#f3f4f6",
  "list.activeSelectionBackground": "#ddf4ff",
  "list.activeSelectionForeground": "#0b3d91",
  "activityBar.background": "#f6f8fa",
  "activityBar.foreground": "#24292f",
  "statusBar.background": "#f6f8fa",
  "statusBar.foreground": "#57606a",
  "titleBar.activeBackground": "#ffffff",
  "titleBar.activeForeground": "#1f2328",
  "terminal.background": "#ffffff",
  "terminal.foreground": "#1f2328",
  "terminal.selectionBackground": "#b6e3ff",
  "terminalCursor.foreground": "#0969da",
  "editorLineNumber.foreground": "#8c959f",
  "editorGutter.addedBackground": "#1a7f37",
  "editorGutter.modifiedBackground": "#0969da",
  "editorGutter.deletedBackground": "#cf222e",
  "textLink.foreground": "#0969da",
  "badge.background": "#d0d7de",
  "badge.foreground": "#1f2328",
};

const SOLARIZED_LIGHT_COLORS: VsCodeThemeColors = {
  foreground: "#657b83",
  descriptionForeground: "#6c7f86",
  "editor.background": "#fdf6e3",
  "editor.foreground": "#586e75",
  "sideBar.background": "#eee8d5",
  "sideBar.foreground": "#586e75",
  "panel.background": "#f7f0dd",
  "panel.border": "#d8cfb5",
  "input.background": "#fdf6e3",
  "input.foreground": "#586e75",
  "input.border": "#cfc7ad",
  "button.background": "#268bd2",
  "button.foreground": "#fdf6e3",
  "button.secondaryBackground": "#eee8d5",
  "button.secondaryForeground": "#586e75",
  "dropdown.background": "#fdf6e3",
  "dropdown.border": "#cfc7ad",
  "editorWidget.background": "#f7f0dd",
  "list.hoverBackground": "#f1ead7",
  "list.activeSelectionBackground": "#dce8ef",
  "list.activeSelectionForeground": "#073642",
  "activityBar.background": "#eee8d5",
  "activityBar.foreground": "#586e75",
  "statusBar.background": "#eee8d5",
  "statusBar.foreground": "#586e75",
  "titleBar.activeBackground": "#fdf6e3",
  "titleBar.activeForeground": "#586e75",
  "terminal.background": "#fdf6e3",
  "terminal.foreground": "#586e75",
  "terminal.selectionBackground": "#dce8ef",
  "terminalCursor.foreground": "#268bd2",
  "editorLineNumber.foreground": "#93a1a1",
  "editorGutter.addedBackground": "#859900",
  "editorGutter.modifiedBackground": "#b58900",
  "editorGutter.deletedBackground": "#dc322f",
  "textLink.foreground": "#268bd2",
  "badge.background": "#e0d8be",
  "badge.foreground": "#586e75",
};

const CATPPUCCIN_LATTE_COLORS: VsCodeThemeColors = {
  foreground: "#4c4f69",
  descriptionForeground: "#6c6f85",
  "editor.background": "#eff1f5",
  "editor.foreground": "#4c4f69",
  "sideBar.background": "#e6e9ef",
  "sideBar.foreground": "#4c4f69",
  "panel.background": "#eff1f5",
  "panel.border": "#ccd0da",
  "input.background": "#e6e9ef",
  "input.foreground": "#4c4f69",
  "input.border": "#bcc0cc",
  "button.background": "#1e66f5",
  "button.foreground": "#ffffff",
  "button.secondaryBackground": "#dce0e8",
  "button.secondaryForeground": "#4c4f69",
  "dropdown.background": "#e6e9ef",
  "dropdown.border": "#bcc0cc",
  "editorWidget.background": "#e6e9ef",
  "list.hoverBackground": "#dce0e8",
  "list.activeSelectionBackground": "#ccd0da",
  "list.activeSelectionForeground": "#1e1e2e",
  "activityBar.background": "#dce0e8",
  "activityBar.foreground": "#4c4f69",
  "statusBar.background": "#e6e9ef",
  "statusBar.foreground": "#4c4f69",
  "titleBar.activeBackground": "#eff1f5",
  "titleBar.activeForeground": "#4c4f69",
  "terminal.background": "#eff1f5",
  "terminal.foreground": "#4c4f69",
  "terminal.selectionBackground": "#ccd0da",
  "terminalCursor.foreground": "#1e66f5",
  "editorLineNumber.foreground": "#8c8fa1",
  "editorGutter.addedBackground": "#40a02b",
  "editorGutter.modifiedBackground": "#df8e1d",
  "editorGutter.deletedBackground": "#d20f39",
  "textLink.foreground": "#1e66f5",
  "badge.background": "#ccd0da",
  "badge.foreground": "#4c4f69",
};

const TOKYO_DAY_COLORS: VsCodeThemeColors = {
  foreground: "#343b58",
  descriptionForeground: "#5a607f",
  "editor.background": "#e1e2e7",
  "editor.foreground": "#343b58",
  "sideBar.background": "#d5d6db",
  "sideBar.foreground": "#343b58",
  "panel.background": "#e9e9ed",
  "panel.border": "#c4c8d8",
  "input.background": "#dfe1e8",
  "input.foreground": "#343b58",
  "input.border": "#b8bfd6",
  "button.background": "#2e7de9",
  "button.foreground": "#ffffff",
  "button.secondaryBackground": "#cfd2e1",
  "button.secondaryForeground": "#343b58",
  "dropdown.background": "#dfe1e8",
  "dropdown.border": "#b8bfd6",
  "editorWidget.background": "#dfe1e8",
  "list.hoverBackground": "#d5d6db",
  "list.activeSelectionBackground": "#c6d7ff",
  "list.activeSelectionForeground": "#1f2335",
  "activityBar.background": "#cfd2e1",
  "activityBar.foreground": "#343b58",
  "statusBar.background": "#d5d6db",
  "statusBar.foreground": "#343b58",
  "titleBar.activeBackground": "#e1e2e7",
  "titleBar.activeForeground": "#343b58",
  "terminal.background": "#e1e2e7",
  "terminal.foreground": "#343b58",
  "terminal.selectionBackground": "#c6d7ff",
  "terminalCursor.foreground": "#2e7de9",
  "editorLineNumber.foreground": "#8990b3",
  "editorGutter.addedBackground": "#587539",
  "editorGutter.modifiedBackground": "#8c6c3e",
  "editorGutter.deletedBackground": "#c64343",
  "textLink.foreground": "#2e7de9",
  "badge.background": "#c4c8d8",
  "badge.foreground": "#343b58",
};

const ROSE_PINE_DAWN_COLORS: VsCodeThemeColors = {
  foreground: "#575279",
  descriptionForeground: "#797593",
  "editor.background": "#faf4ed",
  "editor.foreground": "#575279",
  "sideBar.background": "#fffaf3",
  "sideBar.foreground": "#575279",
  "panel.background": "#f2e9e1",
  "panel.border": "#dfdad9",
  "input.background": "#fffaf3",
  "input.foreground": "#575279",
  "input.border": "#cecacd",
  "button.background": "#286983",
  "button.foreground": "#fffaf3",
  "button.secondaryBackground": "#f2e9e1",
  "button.secondaryForeground": "#575279",
  "dropdown.background": "#fffaf3",
  "dropdown.border": "#cecacd",
  "editorWidget.background": "#fffaf3",
  "list.hoverBackground": "#f2e9e1",
  "list.activeSelectionBackground": "#eadfd7",
  "list.activeSelectionForeground": "#575279",
  "activityBar.background": "#f2e9e1",
  "activityBar.foreground": "#575279",
  "statusBar.background": "#f2e9e1",
  "statusBar.foreground": "#575279",
  "titleBar.activeBackground": "#faf4ed",
  "titleBar.activeForeground": "#575279",
  "terminal.background": "#faf4ed",
  "terminal.foreground": "#575279",
  "terminal.selectionBackground": "#eadfd7",
  "terminalCursor.foreground": "#286983",
  "editorLineNumber.foreground": "#9893a5",
  "editorGutter.addedBackground": "#56949f",
  "editorGutter.modifiedBackground": "#ea9d34",
  "editorGutter.deletedBackground": "#b4637a",
  "textLink.foreground": "#286983",
  "badge.background": "#dfdad9",
  "badge.foreground": "#575279",
};

const EVERFOREST_LIGHT_COLORS: VsCodeThemeColors = {
  foreground: "#5c6a72",
  descriptionForeground: "#7a8478",
  "editor.background": "#fdf6e3",
  "editor.foreground": "#5c6a72",
  "sideBar.background": "#efebd4",
  "sideBar.foreground": "#5c6a72",
  "panel.background": "#f4f0d9",
  "panel.border": "#d8d3ba",
  "input.background": "#efebd4",
  "input.foreground": "#5c6a72",
  "input.border": "#c9c2a7",
  "button.background": "#3a94c5",
  "button.foreground": "#fff9e8",
  "button.secondaryBackground": "#e6e2cc",
  "button.secondaryForeground": "#5c6a72",
  "dropdown.background": "#efebd4",
  "dropdown.border": "#c9c2a7",
  "editorWidget.background": "#efebd4",
  "list.hoverBackground": "#e6e2cc",
  "list.activeSelectionBackground": "#d8d3ba",
  "list.activeSelectionForeground": "#3c4841",
  "activityBar.background": "#e6e2cc",
  "activityBar.foreground": "#5c6a72",
  "statusBar.background": "#efebd4",
  "statusBar.foreground": "#5c6a72",
  "titleBar.activeBackground": "#fdf6e3",
  "titleBar.activeForeground": "#5c6a72",
  "terminal.background": "#fdf6e3",
  "terminal.foreground": "#5c6a72",
  "terminal.selectionBackground": "#d8d3ba",
  "terminalCursor.foreground": "#3a94c5",
  "editorLineNumber.foreground": "#a6b0a0",
  "editorGutter.addedBackground": "#8da101",
  "editorGutter.modifiedBackground": "#dfa000",
  "editorGutter.deletedBackground": "#f85552",
  "textLink.foreground": "#3a94c5",
  "badge.background": "#d8d3ba",
  "badge.foreground": "#5c6a72",
};

const AYU_LIGHT_COLORS: VsCodeThemeColors = {
  foreground: "#5c6773",
  descriptionForeground: "#828c99",
  "editor.background": "#fafafa",
  "editor.foreground": "#5c6773",
  "sideBar.background": "#f0f0f0",
  "sideBar.foreground": "#5c6773",
  "panel.background": "#f5f5f5",
  "panel.border": "#d9d9d9",
  "input.background": "#ffffff",
  "input.foreground": "#5c6773",
  "input.border": "#d9d9d9",
  "button.background": "#55b4d4",
  "button.foreground": "#ffffff",
  "button.secondaryBackground": "#e8e8e8",
  "button.secondaryForeground": "#5c6773",
  "dropdown.background": "#ffffff",
  "dropdown.border": "#d9d9d9",
  "editorWidget.background": "#ffffff",
  "list.hoverBackground": "#eeeeee",
  "list.activeSelectionBackground": "#d3edf6",
  "list.activeSelectionForeground": "#263238",
  "activityBar.background": "#e8e8e8",
  "activityBar.foreground": "#5c6773",
  "statusBar.background": "#f0f0f0",
  "statusBar.foreground": "#5c6773",
  "titleBar.activeBackground": "#fafafa",
  "titleBar.activeForeground": "#5c6773",
  "terminal.background": "#fafafa",
  "terminal.foreground": "#5c6773",
  "terminal.selectionBackground": "#d3edf6",
  "terminalCursor.foreground": "#55b4d4",
  "editorLineNumber.foreground": "#abb0b6",
  "editorGutter.addedBackground": "#86b300",
  "editorGutter.modifiedBackground": "#f29718",
  "editorGutter.deletedBackground": "#f07171",
  "textLink.foreground": "#55b4d4",
  "badge.background": "#d9d9d9",
  "badge.foreground": "#5c6773",
};

const GITHUB_DARK_COLORS: VsCodeThemeColors = {
  foreground: "#c9d1d9",
  descriptionForeground: "#8b949e",
  "editor.background": "#0d1117",
  "editor.foreground": "#c9d1d9",
  "sideBar.background": "#010409",
  "sideBar.foreground": "#c9d1d9",
  "panel.background": "#0d1117",
  "panel.border": "#30363d",
  "input.background": "#161b22",
  "input.foreground": "#c9d1d9",
  "input.border": "#30363d",
  "button.background": "#2f81f7",
  "button.foreground": "#ffffff",
  "button.secondaryBackground": "#21262d",
  "button.secondaryForeground": "#c9d1d9",
  "dropdown.background": "#161b22",
  "dropdown.border": "#30363d",
  "editorWidget.background": "#161b22",
  "list.hoverBackground": "#161b22",
  "list.activeSelectionBackground": "#1f6feb",
  "list.activeSelectionForeground": "#ffffff",
  "activityBar.background": "#010409",
  "activityBar.foreground": "#f0f6fc",
  "statusBar.background": "#161b22",
  "statusBar.foreground": "#c9d1d9",
  "titleBar.activeBackground": "#0d1117",
  "titleBar.activeForeground": "#c9d1d9",
  "terminal.background": "#0d1117",
  "terminal.foreground": "#c9d1d9",
  "terminal.selectionBackground": "#264f78",
  "terminalCursor.foreground": "#c9d1d9",
  "editorLineNumber.foreground": "#6e7681",
  "editorGutter.addedBackground": "#3fb950",
  "editorGutter.modifiedBackground": "#2f81f7",
  "editorGutter.deletedBackground": "#f85149",
  "textLink.foreground": "#58a6ff",
  "badge.background": "#30363d",
  "badge.foreground": "#f0f6fc",
};

const GITHUB_DARK_DIMMED_COLORS: VsCodeThemeColors = {
  foreground: "#adbac7",
  descriptionForeground: "#768390",
  "editor.background": "#22272e",
  "editor.foreground": "#adbac7",
  "sideBar.background": "#1c2128",
  "sideBar.foreground": "#adbac7",
  "panel.background": "#22272e",
  "panel.border": "#373e47",
  "input.background": "#2d333b",
  "input.foreground": "#adbac7",
  "input.border": "#444c56",
  "button.background": "#539bf5",
  "button.foreground": "#ffffff",
  "button.secondaryBackground": "#373e47",
  "button.secondaryForeground": "#adbac7",
  "dropdown.background": "#2d333b",
  "dropdown.border": "#444c56",
  "editorWidget.background": "#2d333b",
  "list.hoverBackground": "#2d333b",
  "list.activeSelectionBackground": "#316dca",
  "list.activeSelectionForeground": "#ffffff",
  "activityBar.background": "#1c2128",
  "activityBar.foreground": "#adbac7",
  "statusBar.background": "#22272e",
  "statusBar.foreground": "#adbac7",
  "titleBar.activeBackground": "#22272e",
  "titleBar.activeForeground": "#adbac7",
  "terminal.background": "#22272e",
  "terminal.foreground": "#adbac7",
  "terminal.selectionBackground": "#264f78",
  "terminalCursor.foreground": "#adbac7",
  "editorLineNumber.foreground": "#636e7b",
  "editorGutter.addedBackground": "#57ab5a",
  "editorGutter.modifiedBackground": "#539bf5",
  "editorGutter.deletedBackground": "#e5534b",
  "textLink.foreground": "#539bf5",
  "badge.background": "#444c56",
  "badge.foreground": "#f0f6fc",
};

const ONE_DARK_PRO_COLORS: VsCodeThemeColors = {
  foreground: "#abb2bf",
  descriptionForeground: "#7f848e",
  "editor.background": "#282c34",
  "editor.foreground": "#abb2bf",
  "sideBar.background": "#21252b",
  "sideBar.foreground": "#abb2bf",
  "panel.background": "#282c34",
  "panel.border": "#3b4048",
  "input.background": "#1e2228",
  "input.foreground": "#abb2bf",
  "input.border": "#3b4048",
  "button.background": "#61afef",
  "button.foreground": "#0b0f14",
  "button.secondaryBackground": "#2c313c",
  "button.secondaryForeground": "#abb2bf",
  "dropdown.background": "#21252b",
  "dropdown.border": "#3b4048",
  "editorWidget.background": "#21252b",
  "list.hoverBackground": "#2c313c",
  "list.activeSelectionBackground": "#3e4451",
  "list.activeSelectionForeground": "#ffffff",
  "activityBar.background": "#21252b",
  "activityBar.foreground": "#d7dae0",
  "statusBar.background": "#21252b",
  "statusBar.foreground": "#abb2bf",
  "titleBar.activeBackground": "#282c34",
  "titleBar.activeForeground": "#abb2bf",
  "terminal.background": "#282c34",
  "terminal.foreground": "#abb2bf",
  "terminal.selectionBackground": "#3e4451",
  "terminalCursor.foreground": "#61afef",
  "editorLineNumber.foreground": "#636d83",
  "editorGutter.addedBackground": "#98c379",
  "editorGutter.modifiedBackground": "#e5c07b",
  "editorGutter.deletedBackground": "#e06c75",
  "textLink.foreground": "#61afef",
  "badge.background": "#3e4451",
  "badge.foreground": "#ffffff",
};

const MONOKAI_COLORS: VsCodeThemeColors = {
  foreground: "#f8f8f2",
  descriptionForeground: "#b6b6aa",
  "editor.background": "#272822",
  "editor.foreground": "#f8f8f2",
  "sideBar.background": "#1e1f1c",
  "sideBar.foreground": "#f8f8f2",
  "panel.background": "#272822",
  "panel.border": "#414339",
  "input.background": "#3b3a32",
  "input.foreground": "#f8f8f2",
  "input.border": "#5b5c57",
  "button.background": "#66d9ef",
  "button.foreground": "#1a1b17",
  "button.secondaryBackground": "#414339",
  "button.secondaryForeground": "#f8f8f2",
  "dropdown.background": "#3b3a32",
  "dropdown.border": "#5b5c57",
  "editorWidget.background": "#1e1f1c",
  "list.hoverBackground": "#3b3a32",
  "list.activeSelectionBackground": "#49483e",
  "list.activeSelectionForeground": "#f8f8f2",
  "activityBar.background": "#1e1f1c",
  "activityBar.foreground": "#f8f8f2",
  "statusBar.background": "#414339",
  "statusBar.foreground": "#f8f8f2",
  "titleBar.activeBackground": "#272822",
  "titleBar.activeForeground": "#f8f8f2",
  "terminal.background": "#272822",
  "terminal.foreground": "#f8f8f2",
  "terminal.selectionBackground": "#49483e",
  "terminalCursor.foreground": "#f8f8f2",
  "editorLineNumber.foreground": "#90908a",
  "editorGutter.addedBackground": "#a6e22e",
  "editorGutter.modifiedBackground": "#e6db74",
  "editorGutter.deletedBackground": "#f92672",
  "textLink.foreground": "#66d9ef",
  "badge.background": "#5b5c57",
  "badge.foreground": "#f8f8f2",
};

const SOLARIZED_DARK_COLORS: VsCodeThemeColors = {
  foreground: "#839496",
  descriptionForeground: "#72898f",
  "editor.background": "#002b36",
  "editor.foreground": "#839496",
  "sideBar.background": "#073642",
  "sideBar.foreground": "#839496",
  "panel.background": "#002b36",
  "panel.border": "#17454f",
  "input.background": "#073642",
  "input.foreground": "#839496",
  "input.border": "#17454f",
  "button.background": "#268bd2",
  "button.foreground": "#fdf6e3",
  "button.secondaryBackground": "#17454f",
  "button.secondaryForeground": "#93a1a1",
  "dropdown.background": "#073642",
  "dropdown.border": "#17454f",
  "editorWidget.background": "#073642",
  "list.hoverBackground": "#0b3a47",
  "list.activeSelectionBackground": "#0f4c5c",
  "list.activeSelectionForeground": "#fdf6e3",
  "activityBar.background": "#073642",
  "activityBar.foreground": "#93a1a1",
  "statusBar.background": "#073642",
  "statusBar.foreground": "#93a1a1",
  "titleBar.activeBackground": "#002b36",
  "titleBar.activeForeground": "#93a1a1",
  "terminal.background": "#002b36",
  "terminal.foreground": "#839496",
  "terminal.selectionBackground": "#0f4c5c",
  "terminalCursor.foreground": "#268bd2",
  "editorLineNumber.foreground": "#586e75",
  "editorGutter.addedBackground": "#859900",
  "editorGutter.modifiedBackground": "#b58900",
  "editorGutter.deletedBackground": "#dc322f",
  "textLink.foreground": "#268bd2",
  "badge.background": "#17454f",
  "badge.foreground": "#93a1a1",
};

const DRACULA_COLORS: VsCodeThemeColors = {
  foreground: "#f8f8f2",
  descriptionForeground: "#bfb7d5",
  "editor.background": "#282a36",
  "editor.foreground": "#f8f8f2",
  "sideBar.background": "#21222c",
  "sideBar.foreground": "#f8f8f2",
  "panel.background": "#282a36",
  "panel.border": "#44475a",
  "input.background": "#44475a",
  "input.foreground": "#f8f8f2",
  "input.border": "#6272a4",
  "button.background": "#bd93f9",
  "button.foreground": "#282a36",
  "button.secondaryBackground": "#44475a",
  "button.secondaryForeground": "#f8f8f2",
  "dropdown.background": "#343746",
  "dropdown.border": "#6272a4",
  "editorWidget.background": "#343746",
  "list.hoverBackground": "#343746",
  "list.activeSelectionBackground": "#44475a",
  "list.activeSelectionForeground": "#f8f8f2",
  "activityBar.background": "#21222c",
  "activityBar.foreground": "#f8f8f2",
  "statusBar.background": "#191a21",
  "statusBar.foreground": "#f8f8f2",
  "titleBar.activeBackground": "#282a36",
  "titleBar.activeForeground": "#f8f8f2",
  "terminal.background": "#282a36",
  "terminal.foreground": "#f8f8f2",
  "terminal.selectionBackground": "#44475a",
  "terminalCursor.foreground": "#f8f8f2",
  "editorLineNumber.foreground": "#6272a4",
  "editorGutter.addedBackground": "#50fa7b",
  "editorGutter.modifiedBackground": "#f1fa8c",
  "editorGutter.deletedBackground": "#ff5555",
  "textLink.foreground": "#8be9fd",
  "badge.background": "#44475a",
  "badge.foreground": "#f8f8f2",
};

const NORD_COLORS: VsCodeThemeColors = {
  foreground: "#d8dee9",
  descriptionForeground: "#aeb7c5",
  "editor.background": "#2e3440",
  "editor.foreground": "#d8dee9",
  "sideBar.background": "#242933",
  "sideBar.foreground": "#d8dee9",
  "panel.background": "#2e3440",
  "panel.border": "#4c566a",
  "input.background": "#3b4252",
  "input.foreground": "#d8dee9",
  "input.border": "#4c566a",
  "button.background": "#5e81ac",
  "button.foreground": "#eceff4",
  "button.secondaryBackground": "#3b4252",
  "button.secondaryForeground": "#d8dee9",
  "dropdown.background": "#3b4252",
  "dropdown.border": "#4c566a",
  "editorWidget.background": "#3b4252",
  "list.hoverBackground": "#3b4252",
  "list.activeSelectionBackground": "#434c5e",
  "list.activeSelectionForeground": "#eceff4",
  "activityBar.background": "#242933",
  "activityBar.foreground": "#eceff4",
  "statusBar.background": "#242933",
  "statusBar.foreground": "#d8dee9",
  "titleBar.activeBackground": "#2e3440",
  "titleBar.activeForeground": "#d8dee9",
  "terminal.background": "#2e3440",
  "terminal.foreground": "#d8dee9",
  "terminal.selectionBackground": "#434c5e",
  "terminalCursor.foreground": "#88c0d0",
  "editorLineNumber.foreground": "#6f7d95",
  "editorGutter.addedBackground": "#a3be8c",
  "editorGutter.modifiedBackground": "#ebcb8b",
  "editorGutter.deletedBackground": "#bf616a",
  "textLink.foreground": "#88c0d0",
  "badge.background": "#4c566a",
  "badge.foreground": "#eceff4",
};

const CATPPUCCIN_MOCHA_COLORS: VsCodeThemeColors = {
  foreground: "#cdd6f4",
  descriptionForeground: "#a6adc8",
  "editor.background": "#1e1e2e",
  "editor.foreground": "#cdd6f4",
  "sideBar.background": "#181825",
  "sideBar.foreground": "#cdd6f4",
  "panel.background": "#1e1e2e",
  "panel.border": "#313244",
  "input.background": "#313244",
  "input.foreground": "#cdd6f4",
  "input.border": "#45475a",
  "button.background": "#89b4fa",
  "button.foreground": "#11111b",
  "button.secondaryBackground": "#313244",
  "button.secondaryForeground": "#cdd6f4",
  "dropdown.background": "#313244",
  "dropdown.border": "#45475a",
  "editorWidget.background": "#313244",
  "list.hoverBackground": "#313244",
  "list.activeSelectionBackground": "#45475a",
  "list.activeSelectionForeground": "#cdd6f4",
  "activityBar.background": "#181825",
  "activityBar.foreground": "#cdd6f4",
  "statusBar.background": "#181825",
  "statusBar.foreground": "#cdd6f4",
  "titleBar.activeBackground": "#1e1e2e",
  "titleBar.activeForeground": "#cdd6f4",
  "terminal.background": "#1e1e2e",
  "terminal.foreground": "#cdd6f4",
  "terminal.selectionBackground": "#45475a",
  "terminalCursor.foreground": "#f5e0dc",
  "editorLineNumber.foreground": "#6c7086",
  "editorGutter.addedBackground": "#a6e3a1",
  "editorGutter.modifiedBackground": "#f9e2af",
  "editorGutter.deletedBackground": "#f38ba8",
  "textLink.foreground": "#89b4fa",
  "badge.background": "#45475a",
  "badge.foreground": "#cdd6f4",
};

const TOKYO_NIGHT_COLORS: VsCodeThemeColors = {
  foreground: "#c0caf5",
  descriptionForeground: "#9aa5ce",
  "editor.background": "#1a1b26",
  "editor.foreground": "#c0caf5",
  "sideBar.background": "#16161e",
  "sideBar.foreground": "#c0caf5",
  "panel.background": "#1a1b26",
  "panel.border": "#2f3549",
  "input.background": "#24283b",
  "input.foreground": "#c0caf5",
  "input.border": "#414868",
  "button.background": "#7aa2f7",
  "button.foreground": "#16161e",
  "button.secondaryBackground": "#24283b",
  "button.secondaryForeground": "#c0caf5",
  "dropdown.background": "#24283b",
  "dropdown.border": "#414868",
  "editorWidget.background": "#24283b",
  "list.hoverBackground": "#24283b",
  "list.activeSelectionBackground": "#2f3549",
  "list.activeSelectionForeground": "#c0caf5",
  "activityBar.background": "#16161e",
  "activityBar.foreground": "#c0caf5",
  "statusBar.background": "#16161e",
  "statusBar.foreground": "#c0caf5",
  "titleBar.activeBackground": "#1a1b26",
  "titleBar.activeForeground": "#c0caf5",
  "terminal.background": "#1a1b26",
  "terminal.foreground": "#c0caf5",
  "terminal.selectionBackground": "#2f3549",
  "terminalCursor.foreground": "#c0caf5",
  "editorLineNumber.foreground": "#565f89",
  "editorGutter.addedBackground": "#9ece6a",
  "editorGutter.modifiedBackground": "#e0af68",
  "editorGutter.deletedBackground": "#f7768e",
  "textLink.foreground": "#7aa2f7",
  "badge.background": "#414868",
  "badge.foreground": "#c0caf5",
};

const ROSE_PINE_COLORS: VsCodeThemeColors = {
  foreground: "#e0def4",
  descriptionForeground: "#908caa",
  "editor.background": "#191724",
  "editor.foreground": "#e0def4",
  "sideBar.background": "#1f1d2e",
  "sideBar.foreground": "#e0def4",
  "panel.background": "#191724",
  "panel.border": "#403d52",
  "input.background": "#26233a",
  "input.foreground": "#e0def4",
  "input.border": "#403d52",
  "button.background": "#ebbcba",
  "button.foreground": "#191724",
  "button.secondaryBackground": "#26233a",
  "button.secondaryForeground": "#e0def4",
  "dropdown.background": "#26233a",
  "dropdown.border": "#403d52",
  "editorWidget.background": "#26233a",
  "list.hoverBackground": "#26233a",
  "list.activeSelectionBackground": "#403d52",
  "list.activeSelectionForeground": "#e0def4",
  "activityBar.background": "#1f1d2e",
  "activityBar.foreground": "#e0def4",
  "statusBar.background": "#1f1d2e",
  "statusBar.foreground": "#e0def4",
  "titleBar.activeBackground": "#191724",
  "titleBar.activeForeground": "#e0def4",
  "terminal.background": "#191724",
  "terminal.foreground": "#e0def4",
  "terminal.selectionBackground": "#403d52",
  "terminalCursor.foreground": "#ebbcba",
  "editorLineNumber.foreground": "#6e6a86",
  "editorGutter.addedBackground": "#31748f",
  "editorGutter.modifiedBackground": "#f6c177",
  "editorGutter.deletedBackground": "#eb6f92",
  "textLink.foreground": "#9ccfd8",
  "badge.background": "#403d52",
  "badge.foreground": "#e0def4",
};

export const PRESET_SYNTAX_TOKENS: Record<ThemePresetId, SyntaxTokens> = {
  "vscode-dark-modern": {
    keyword: "#569cd6",
    string: "#ce9178",
    comment: "#6a9955",
    number: "#b5cea8",
    function: "#dcdcaa",
    operator: "#d4d4d4",
    type: "#4ec9b0",
    tag: "#569cd6",
  },
  "vscode-dark-plus": {
    keyword: "#569cd6",
    string: "#ce9178",
    comment: "#6a9955",
    number: "#b5cea8",
    function: "#dcdcaa",
    operator: "#d4d4d4",
    type: "#4ec9b0",
    tag: "#569cd6",
  },
  "vscode-light-modern": {
    keyword: "#0000ff",
    string: "#a31515",
    comment: "#008000",
    number: "#098658",
    function: "#795e26",
    operator: "#000000",
    type: "#267f99",
    tag: "#0000ff",
  },
  "vscode-light-plus": {
    keyword: "#0000ff",
    string: "#a31515",
    comment: "#008000",
    number: "#098658",
    function: "#795e26",
    operator: "#000000",
    type: "#267f99",
    tag: "#0000ff",
  },
  "vscode-github-light": {
    keyword: "#cf222e",
    string: "#0a3069",
    comment: "#6e7781",
    number: "#0550ae",
    function: "#8250df",
    operator: "#24292f",
    type: "#953800",
    tag: "#116329",
  },
  "vscode-solarized-light": {
    keyword: "#859900",
    string: "#2aa198",
    comment: "#93a1a1",
    number: "#d33682",
    function: "#268bd2",
    operator: "#586e75",
    type: "#b58900",
    tag: "#268bd2",
  },
  "vscode-catppuccin-latte": {
    keyword: "#8839ef",
    string: "#40a02b",
    comment: "#9ca0b0",
    number: "#fe640b",
    function: "#1e66f5",
    operator: "#7c7f93",
    type: "#df8e1d",
    tag: "#dd7878",
  },
  "vscode-tokyo-day": {
    keyword: "#343b58",
    string: "#587539",
    comment: "#848cb5",
    number: "#965027",
    function: "#166775",
    operator: "#343b58",
    type: "#b15c00",
    tag: "#343b58",
  },
  "vscode-rose-pine-dawn": {
    keyword: "#286983",
    string: "#d7827e",
    comment: "#9893a5",
    number: "#ea9d34",
    function: "#907aa9",
    operator: "#5c5a77",
    type: "#d7827e",
    tag: "#286983",
  },
  "vscode-everforest-light": {
    keyword: "#8da101",
    string: "#7a8943",
    comment: "#a0b58c",
    number: "#df69ba",
    function: "#3a94c5",
    operator: "#5c6a72",
    type: "#b47152",
    tag: "#8da101",
  },
  "vscode-ayu-light": {
    keyword: "#a626a4",
    string: "#50a04f",
    comment: "#a0ada0",
    number: "#b75c00",
    function: "#399ee6",
    operator: "#5c6166",
    type: "#a626a4",
    tag: "#a626a4",
  },
  "vscode-github-dark": {
    keyword: "#ff7b72",
    string: "#a5d6ff",
    comment: "#8b949e",
    number: "#79c0ff",
    function: "#d2a8ff",
    operator: "#c9d1d9",
    type: "#ffa657",
    tag: "#7ee787",
  },
  "vscode-github-dark-dimmed": {
    keyword: "#f47067",
    string: "#96cbca",
    comment: "#8b949e",
    number: "#6cb6ff",
    function: "#b392f0",
    operator: "#9d9d9d",
    type: "#c69026",
    tag: "#adbac7",
  },
  "vscode-one-dark-pro": {
    keyword: "#c678dd",
    string: "#98c379",
    comment: "#5c6370",
    number: "#d19a66",
    function: "#61afef",
    operator: "#56b6c2",
    type: "#e5c07b",
    tag: "#e06c75",
  },
  "vscode-monokai": {
    keyword: "#f92672",
    string: "#e6db74",
    comment: "#75715e",
    number: "#ae81ff",
    function: "#a6e22e",
    operator: "#f92672",
    type: "#66d9ef",
    tag: "#f92672",
  },
  "vscode-solarized-dark": {
    keyword: "#859900",
    string: "#2aa198",
    comment: "#586e75",
    number: "#d33682",
    function: "#268bd2",
    operator: "#93a1a1",
    type: "#b58900",
    tag: "#268bd2",
  },
  "vscode-dracula": {
    keyword: "#ff79c6",
    string: "#f1fa8c",
    comment: "#6272a4",
    number: "#bd93f9",
    function: "#50fa7b",
    operator: "#ff79c6",
    type: "#8be9fd",
    tag: "#ff79c6",
  },
  "vscode-nord": {
    keyword: "#81a1c1",
    string: "#a3be8c",
    comment: "#616e88",
    number: "#b48ead",
    function: "#88c0d0",
    operator: "#81a1c1",
    type: "#ebcb8b",
    tag: "#81a1c1",
  },
  "vscode-catppuccin-mocha": {
    keyword: "#cba6f7",
    string: "#a6e3a1",
    comment: "#6c7086",
    number: "#fab387",
    function: "#89b4fa",
    operator: "#94e2d5",
    type: "#f9e2af",
    tag: "#f38ba8",
  },
  "vscode-tokyo-night": {
    keyword: "#bb9af7",
    string: "#9ece6a",
    comment: "#565f89",
    number: "#ff9e64",
    function: "#7aa2f7",
    operator: "#89ddff",
    type: "#7dcfff",
    tag: "#f7768e",
  },
  "vscode-rose-pine": {
    keyword: "#c4a7e7",
    string: "#9ccfd8",
    comment: "#6e6a86",
    number: "#ebbcba",
    function: "#ebbcba",
    operator: "#908caa",
    type: "#f6c177",
    tag: "#eb6f92",
  },
};

export const PRESET_DIFF_TOKENS: Record<ThemePresetId, DiffTokens> = {
  "vscode-dark-modern": { inserted: "#2ea043", removed: "#f85149" },
  "vscode-dark-plus": { inserted: "#2ea043", removed: "#f14c4c" },
  "vscode-light-modern": { inserted: "#2ea043", removed: "#f85149" },
  "vscode-light-plus": { inserted: "#2ea043", removed: "#cd3131" },
  "vscode-github-light": { inserted: "#1a7f37", removed: "#cf222e" },
  "vscode-solarized-light": { inserted: "#859900", removed: "#dc322f" },
  "vscode-catppuccin-latte": { inserted: "#40a02b", removed: "#d20f39" },
  "vscode-tokyo-day": { inserted: "#587539", removed: "#c64343" },
  "vscode-rose-pine-dawn": { inserted: "#56949f", removed: "#b4637a" },
  "vscode-everforest-light": { inserted: "#8da101", removed: "#f85552" },
  "vscode-ayu-light": { inserted: "#86b300", removed: "#f07171" },
  "vscode-github-dark": { inserted: "#3fb950", removed: "#f85149" },
  "vscode-github-dark-dimmed": { inserted: "#57ab5a", removed: "#e5534b" },
  "vscode-one-dark-pro": { inserted: "#98c379", removed: "#e06c75" },
  "vscode-monokai": { inserted: "#a6e22e", removed: "#f92672" },
  "vscode-solarized-dark": { inserted: "#859900", removed: "#dc322f" },
  "vscode-dracula": { inserted: "#50fa7b", removed: "#ff5555" },
  "vscode-nord": { inserted: "#a3be8c", removed: "#bf616a" },
  "vscode-catppuccin-mocha": { inserted: "#a6e3a1", removed: "#f38ba8" },
  "vscode-tokyo-night": { inserted: "#9ece6a", removed: "#f7768e" },
  "vscode-rose-pine": { inserted: "#9ccfd8", removed: "#eb6f92" },
};

export const VSCODE_THEME_PRESETS: Record<
  ThemePresetId,
  VsCodeThemePresetDefinition
> = {
  "vscode-dark-modern": {
    id: "vscode-dark-modern",
    appearance: "dark",
    labelKey: "settings.themePresetDarkModern",
    colors: DARK_MODERN_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-dark-modern"],
    diff: PRESET_DIFF_TOKENS["vscode-dark-modern"],
  },
  "vscode-dark-plus": {
    id: "vscode-dark-plus",
    appearance: "dark",
    labelKey: "settings.themePresetDarkPlus",
    colors: DARK_PLUS_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-dark-plus"],
    diff: PRESET_DIFF_TOKENS["vscode-dark-plus"],
  },
  "vscode-light-modern": {
    id: "vscode-light-modern",
    appearance: "light",
    labelKey: "settings.themePresetLightModern",
    colors: LIGHT_MODERN_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-light-modern"],
    diff: PRESET_DIFF_TOKENS["vscode-light-modern"],
  },
  "vscode-light-plus": {
    id: "vscode-light-plus",
    appearance: "light",
    labelKey: "settings.themePresetLightPlus",
    colors: LIGHT_PLUS_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-light-plus"],
    diff: PRESET_DIFF_TOKENS["vscode-light-plus"],
  },
  "vscode-github-light": {
    id: "vscode-github-light",
    appearance: "light",
    labelKey: "settings.themePresetGitHubLight",
    colors: GITHUB_LIGHT_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-github-light"],
    diff: PRESET_DIFF_TOKENS["vscode-github-light"],
  },
  "vscode-solarized-light": {
    id: "vscode-solarized-light",
    appearance: "light",
    labelKey: "settings.themePresetSolarizedLight",
    colors: SOLARIZED_LIGHT_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-solarized-light"],
    diff: PRESET_DIFF_TOKENS["vscode-solarized-light"],
  },
  "vscode-catppuccin-latte": {
    id: "vscode-catppuccin-latte",
    appearance: "light",
    labelKey: "settings.themePresetCatppuccinLatte",
    colors: CATPPUCCIN_LATTE_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-catppuccin-latte"],
    diff: PRESET_DIFF_TOKENS["vscode-catppuccin-latte"],
  },
  "vscode-tokyo-day": {
    id: "vscode-tokyo-day",
    appearance: "light",
    labelKey: "settings.themePresetTokyoDay",
    colors: TOKYO_DAY_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-tokyo-day"],
    diff: PRESET_DIFF_TOKENS["vscode-tokyo-day"],
  },
  "vscode-rose-pine-dawn": {
    id: "vscode-rose-pine-dawn",
    appearance: "light",
    labelKey: "settings.themePresetRosePineDawn",
    colors: ROSE_PINE_DAWN_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-rose-pine-dawn"],
    diff: PRESET_DIFF_TOKENS["vscode-rose-pine-dawn"],
  },
  "vscode-everforest-light": {
    id: "vscode-everforest-light",
    appearance: "light",
    labelKey: "settings.themePresetEverforestLight",
    colors: EVERFOREST_LIGHT_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-everforest-light"],
    diff: PRESET_DIFF_TOKENS["vscode-everforest-light"],
  },
  "vscode-ayu-light": {
    id: "vscode-ayu-light",
    appearance: "light",
    labelKey: "settings.themePresetAyuLight",
    colors: AYU_LIGHT_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-ayu-light"],
    diff: PRESET_DIFF_TOKENS["vscode-ayu-light"],
  },
  "vscode-github-dark": {
    id: "vscode-github-dark",
    appearance: "dark",
    labelKey: "settings.themePresetGitHubDark",
    colors: GITHUB_DARK_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-github-dark"],
    diff: PRESET_DIFF_TOKENS["vscode-github-dark"],
  },
  "vscode-github-dark-dimmed": {
    id: "vscode-github-dark-dimmed",
    appearance: "dark",
    labelKey: "settings.themePresetGitHubDarkDimmed",
    colors: GITHUB_DARK_DIMMED_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-github-dark-dimmed"],
    diff: PRESET_DIFF_TOKENS["vscode-github-dark-dimmed"],
  },
  "vscode-one-dark-pro": {
    id: "vscode-one-dark-pro",
    appearance: "dark",
    labelKey: "settings.themePresetOneDarkPro",
    colors: ONE_DARK_PRO_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-one-dark-pro"],
    diff: PRESET_DIFF_TOKENS["vscode-one-dark-pro"],
  },
  "vscode-monokai": {
    id: "vscode-monokai",
    appearance: "dark",
    labelKey: "settings.themePresetMonokai",
    colors: MONOKAI_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-monokai"],
    diff: PRESET_DIFF_TOKENS["vscode-monokai"],
  },
  "vscode-solarized-dark": {
    id: "vscode-solarized-dark",
    appearance: "dark",
    labelKey: "settings.themePresetSolarizedDark",
    colors: SOLARIZED_DARK_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-solarized-dark"],
    diff: PRESET_DIFF_TOKENS["vscode-solarized-dark"],
  },
  "vscode-dracula": {
    id: "vscode-dracula",
    appearance: "dark",
    labelKey: "settings.themePresetDracula",
    colors: DRACULA_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-dracula"],
    diff: PRESET_DIFF_TOKENS["vscode-dracula"],
  },
  "vscode-nord": {
    id: "vscode-nord",
    appearance: "dark",
    labelKey: "settings.themePresetNord",
    colors: NORD_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-nord"],
    diff: PRESET_DIFF_TOKENS["vscode-nord"],
  },
  "vscode-catppuccin-mocha": {
    id: "vscode-catppuccin-mocha",
    appearance: "dark",
    labelKey: "settings.themePresetCatppuccinMocha",
    colors: CATPPUCCIN_MOCHA_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-catppuccin-mocha"],
    diff: PRESET_DIFF_TOKENS["vscode-catppuccin-mocha"],
  },
  "vscode-tokyo-night": {
    id: "vscode-tokyo-night",
    appearance: "dark",
    labelKey: "settings.themePresetTokyoNight",
    colors: TOKYO_NIGHT_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-tokyo-night"],
    diff: PRESET_DIFF_TOKENS["vscode-tokyo-night"],
  },
  "vscode-rose-pine": {
    id: "vscode-rose-pine",
    appearance: "dark",
    labelKey: "settings.themePresetRosePine",
    colors: ROSE_PINE_COLORS,
    syntax: PRESET_SYNTAX_TOKENS["vscode-rose-pine"],
    diff: PRESET_DIFF_TOKENS["vscode-rose-pine"],
  },
};

export function getVsCodeThemePreset(
  presetId: ThemePresetId,
): VsCodeThemePresetDefinition {
  return VSCODE_THEME_PRESETS[presetId];
}
