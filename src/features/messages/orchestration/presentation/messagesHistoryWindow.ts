 ...
 * - 生产默认 150。Task 1 定标（Messages.history-window.test.tsx，真实 Markdown、
 *   正文+工具输出+diff+代码块混合）：单条均值 ≈ 20~26 个 DOM 节点；
 *   150 条窗口 ≈ 3k~3.9k 节点，处于流畅预算内；500 条全量 ≈ 13k 节点且线性增长。
 * - 测试默认 0（关闭=全量），保持既有用例确定；显式 setItem 开启
 * - 值 <=0 或非法 = 关闭（恢复全量）
 */
export const HISTORY_WINDOW_SIZE_FLAG_KEY = "ccgui.perf.historyWindowSize";
export const DEFAULT_HISTORY_WINDOW_SIZE = 150;
